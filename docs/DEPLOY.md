...
Напиши боту в Telegram — должен ответить. Затем отправь `/check`: он опросит подписки сразу и
ответит по каждой — самое быстрое подтверждение, что цепочка «источник → парсер → доставка»

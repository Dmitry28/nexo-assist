...
пинит образ этого же коммита → `kubectl apply -k` → ждёт `rollout status` (под не поднялся —
джоба красная).
...
**Откат** (автомата нет — делается руками, две опции):
...
**Ограничения текущей схемы** (осознанные, см. PRODUCT_PLAN): откат ручной; гейта

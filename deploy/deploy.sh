...
# it through to the deploy ref (`main`), then re-run setup-server.sh on the host: it installs
# what is pushed, not a local edit. The running copy is deliberately NOT the one in the

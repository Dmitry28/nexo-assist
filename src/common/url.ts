...
/** `www.` is not part of a site's identity — drop it before comparing or canonicalizing a host. */
...
export function matchesHost({ url, host }: { url: string; host: string }): boolean {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
